...
namespace DrRobot.JaguarControl
{
    public class Navigation
    {
        ...
        public double safeRadius = 0.6; //initial safe radius
        public double laserLost = 0;

        // TODO MAKE THESE ACTUAL VALUES
        double std_l = .25;// = 0.25; //1.2
        double std_r = .25;// = 0.25; //1.2;
        ...
        public int numParticles_temp = 500;
        public double K_wheelRandomness = 0.15;//0.25
        public Random random = new Random();
        public bool newLaserData = false;
        public double laserMaxRange = 6.0; //TODO (we changed it from 4)
        ...
        public double[,] milestones = new double[,] {{-22,-20.5, 23, 25, 0}, //start (milestone)
           // {-29.5,-27.5, 23.5, 25.5, 0}, // first corner    */
            { -29, -28, 16.5, 17.5, 0 }, // second corner (milestone) 
            { -24.5, -24,   15, 16.5, 0 }, // first pasageway 
            { -22.5, -21,    8, 10, 0 }, //{ -26, -25, 5, 7, 0 }, // in the river (milestone) 
            //{ -24.5,-23,0,-2,0},
            { -22, -20, -3.2, -2, 0 },  // outside pasage (milestone)
            { -25.5, -24.5, -6.5, -4.5, 0 },  // second pasageway (milestone) 
           // { -26.5, -25.5, -2,-1,0}, // point that makes it not face backwards  
            {-6,-5,  -3.2, -2, 0 },     // end of far planter
            {-5.5,-4.5, 7.5,8.5, 0 },      // in front of door (milestone)
            { -4,  -3, 3.5,4.5, 0 },  // corner of planter
            {  5,   6,   4,  6, 0 },  // close to first manhole (milestone?)
            { 25,  24,   5,  4, 0 },  // corner
            { 25,  24,  -9,-10, 0 },  // the suspense (milestone)
            { 24,  23,  -9,-10, 0 }   // broken cuz it fell down stairs. (milestone)
            };
        ...
        public void Initialize()
        {
            ...
            x = -21.5;       // initialX; -21.5
            y = 24.5;      // initialY; 24.5
            t = 3.14;     // initialT; 3.14
...
;                    }
        ...
        public void CalcMotorSignals()
        {
            ...
            // slow down spot turns
            if (Math.Sign(desiredRotRateL * desiredRotRateR) < 0)
            {
                desiredRotRateR = (short)(0.8 * (double)desiredRotRateR);
                desiredRotRateL = (short)(0.8 * (double)desiredRotRateL);
            }
            ...
            motorSignalL = (short)((zeroOutput + desiredRotRateL * 130 + signalL)/ (1.8519 / 1.8519));// (zeroOutput + u_L);
            motorSignalR = (short)((zeroOutput - desiredRotRateR * 130 - signalR)/ (1.8519 / 1.8519));//(zeroOutput - u_R);
        ...
        private void FollowMilestones()
           {
           		...
           		if(x_est > milestones[milestoneNum, 0] && x_est < milestones[milestoneNum, 1] && y_est > milestones[milestoneNum, 2] && y_est < milestones[milestoneNum, 3]){
            ...
            // this is for dynamically changing parameters based on location
...
*/
        ...
        public void LocalizeEstWithParticleFilter()
        {
            ...
            if (milestoneNum == 4 && resetcount < 1)
            {
                x = x_est;
                y = y_est;
                t = t_est;
            ...
            if (milestoneNum > 4 && milestoneNum < 6)
            {
                x_est = x;
                y_est = y;
                t_est = t;
            ...
            if (milestoneNum == 5 && resetcount < 1)
            {
                for(int i=0; i<numParticles; i++){
                    SetOdoPos(i);
                }
        ...
        void CalculateWeight(int p, Tuple<double[], double[]> pillar_corr)
        {
            ...
            if (true)
            {
                ...
                if (pillar_corr.Item1.Length > 0 && false)
                {
        ...
        {
            double[] pillars_x = new double[] { -3.35, 0, 3.35, 7, -3.35, 0, 3.35, 7, 0 };
            double[] pillars_y = new double[] { -9.63, -9.63, -9.63, -9.63, -12.79, -12.79, -12.79, -12.79, -9.63 + 3.16 };
